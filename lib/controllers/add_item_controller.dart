...
class AddItemController extends GetxController {
  ...
  void onInit() {
    ...
    if (Get.arguments != null && Get.arguments is ItemModel) {
      editingItem = Get.arguments as ItemModel;
      titleController.text = editingItem!.title;
      descriptionController.text = editingItem!.description;
      locationController.text = editingItem!.location;
      isLost.value = editingItem!.type == ItemType.lost;
      selectedCategory.value = editingItem!.category;
      if (editingItem!.imageUrls.isNotEmpty) {
        imagePath.value = editingItem!.imageUrls.first;
      }
    }
  }
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      final docId = editingItem != null ? editingItem!.id : FirebaseFirestore.instance.collection('items').doc().id;
      ...
      final newItem = ItemModel(
        ...
        // Keep the old image if no new image was picked, or use the base64 string
        imageUrls: base64ImageUrl != null 
            ? [base64ImageUrl] 
            : (editingItem != null ? editingItem!.imageUrls : []), 
        ownerId: currentUser.id,
        createdAt: editingItem != null ? editingItem!.createdAt : DateTime.now(),
        status: editingItem != null ? editingItem!.status : (isLost.value ? ItemStatus.lost : ItemStatus.found),
      ...
      Get.snackbar(
        'Success', 
        editingItem != null ? 'Item updated successfully!' : 'Item reported successfully!',
